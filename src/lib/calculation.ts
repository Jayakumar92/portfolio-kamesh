...
export const areaOfCrossSection = (doo: number, dia: number): number => {
  return (3.14 / 4) * (Math.pow(doo, 2) - Math.pow(dia, 2))
}

export const momentOfInertiaOfRod = (doo: number, dia: number): number => {
  return (3.14 / 64) * (Math.pow(doo, 2) - Math.pow(dia, 2))
}

export const radiusOfGyration = (i: number, a: number) => {
  return Math.sqrt(i / a)
}

export const slendernessRatio = (le: number, r: number) => {
  return le / r
}
